...
class BaseProperty(metaclass=ABCMeta):
    ...
    def value(self, value, retain=True, qos=1):
        if self.validate_value(value):
            self._value = value
            self.publish(self.topic, self.get_payload_from_value(
                value), retain, qos)
    ...
    # convert payload to a value we can use
    def get_value_from_payload(self, payload):
        return payload

    # convert value to a payload for homie
    ...
    def publish_attributes(self, retain=True, qos=1):
        ...
        self.publish(
            "/".join((self.topic, "$settable")
                     ), str(self.settable).lower(), retain, qos
        )
        self.publish(
            "/".join((self.topic, "$retained")
                     ), str(self.retained).lower(), retain, qos
        )
        if self.unit:
            self.publish("/".join((self.topic, "$unit")),
                         self.unit, retain, qos)
        if self.data_type:
            self.publish(
                "/".join((self.topic, "$datatype")
                         ), self.data_type, retain, qos
            )
        if self.data_format:
            self.publish(
                "/".join((self.topic, "$format")
                         ), self.data_format, retain, qos
            )
    ...
    def set_message_handler(self, topic, payload):
        logger.info(
            "MQTT Property Message:  Topic {}, Payload {}".format(
                topic, payload)
        )
    ...
    def process_set_message(self, topic, payload):  # override as needed
        ...
        if value is not None:
            if self.validate_value(value):
                self.value = value
                # call function to actually change the value
