...
Allows easy use of different MQTT clients.
"""
from abc import ABCMeta, abstractmethod
import logging
from ..helpers.network_information import NetworkInformation

network_info = NetworkInformation()
logger = logging.getLogger(__name__)


class MQTTBase(metaclass=ABCMeta):
    ...
    def connect(self):
        ...
        Notes
        -----
        Called by the device when its ready for the mqtt client to start,
        subclass to provide.
        ...
        logger.debug(
            "MQTT Connecting to {} as client {}".format(
                ...
                self.mqtt_settings["MQTT_CLIENT_ID"]
            )
        )
    ...
    def get_mac_ip_address(self):
        ...
        if self.mac_address is None:
            self.mac_address = network_info.get_local_mac_for_ip(
                self.ip_address)
